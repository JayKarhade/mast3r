...
#     scene = sparse_global_alignment(image_paths, pairs, cache_dir,
...
def get_mast3r_reconstruction(image_paths, model, device='cuda', image_size=512,
                           ...
                           scenegraph_type='complete', winsize=5, refid=0):
    """
    Main function to reconstruct scene from images in a directory
    
    Args:
        image_paths: Path to list of images
        output_dir: Directory to save outputs
        ...
        min_conf_thr: Minimum confidence threshold
        scenegraph_type: Type of scene graph ('complete', 'swin', 'oneref')
        winsize: Window size for scene graph
        refid: Reference ID for scene graph
    ...
    Returns:
        ...
        outfile: Path to output 3D model file
        imgs: List of RGB, depth, and confidence images
    """
    model.eval()
    ...
    if scenegraph_type == "swin":
        scenegraph_type = f"{scenegraph_type}-{winsize}"
    elif scenegraph_type == "oneref":
...
def mast3r_cotracker(args, collected_image_paths, collected_images, collected_query_pts, device="cuda"):
    ...
    mast3r_model = AsymmetricMASt3R.from_pretrained(args.weights).to(args.device)
    mast3r_model.eval()
...
def main():
    ...
    if args.viz:
        rr.script_setup(args, f"Mast3r_CoTracker_Benchmarking_{args.dataset}")
    ...
    for sequence_path in data_sequences:
        ...
        # Run model
        predictions = mast3r_cotracker(args, collected_img_paths, collected_imgs, query_pts, device=device)
